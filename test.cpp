...
public:
    ...
    void id_c(void) { id = y_coordinate * razmer1 + x_coordinate; }
    ...
    double c(NODE &predecessor) {  // distance between two nodes
        ...
        int dd = fabs(dx * dy);
        if (dd > 0)
            return sqrt(2);  // sqrt(2) in case of diagonal transition
        else
            return 1;  // 1 in case of horizontal/vertical transition
    }

    void g_calc(NODE &predecessor) { g = predecessor.g + c(predecessor); }
    /*
     void h_calc(NODE &goal){ // h = e * hypot(abs(x_coordinate -
     goal.x_coordinate), abs(y_coordinate - goal.y_coordinate));
     }
     */

    /*
     void h_calc(NODE &goal){ // h = abs(x_coordinate - goal.x_coordinate) +
     abs(y_coordinate - goal.y_coordinate) + (sqrt(2) - 2)*fmin(abs(x_coordinate
     - goal.x_coordinate),abs(y_coordinate - goal.y_coordinate)); h *= e;
     }
     */

    void h_calc(NODE &goal) {  //
        h = e * fmax(abs(x_coordinate - goal.x_coordinate),
                     abs(y_coordinate - goal.y_coordinate));
    }

    void f_calc() { f = g + h; }

    void check_g(double new_g) { g = fmin(g, new_g); }
};
...
bool equivalent(
    NODE &one,
    NODE &two) {  // Test for two nodes being same (used for goal and start)
    if ((one.x_coordinate == two.x_coordinate) &&
...
void output(unordered_map<int, class NODE> &um) {
    ...
    for (auto it = um.begin(); it != um.end(); ++it) {
        cout << "(" << (*it).second.x_coordinate << ", "
             << (*it).second.y_coordinate << ") ";
    }
    cout << endl << endl;
}
...
// void?
// get successors
vector<vector<NODE>> neighbours_check(NODE &n, vector<vector<NODE>> &neighbours,
    ...
    for (int i = 0; i < 3; ++i){
      for(int j = 0; j < 3; ++j){
        cout << "(" <<neighbours[i][j].x_coordinate << " " <<
    neighbours[i][j].y_coordinate << ")  ";
      }
      cout << endl;
    }*/
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            if (CLOSED.find((neighbours[i][j]).id) != CLOSED.end())
                neighbours[i][j].is_blocked = 1;
            // if(CLOSED.find((neighbours[i][j]).id) != CLOSED.end()) cout <<
            // "found one! this dude is: (" << neighbours[i][j].x_coordinate <<
            // " " << neighbours[i][j].y_coordinate << ")" << endl;
        }
    }
    return neighbours;
}

set<class NODE, comparator> update_open(set<class NODE, comparator> &st_open,
                                        unordered_set<int> &open_id,
                                        NODE &new_node, NODE &n_goal,
                                        NODE *pred) {
    auto open_iterator = st_open.begin();

    auto is_inside_open = open_id.find(new_node.id);
    if (is_inside_open != open_id.end()) {
        while (((*open_iterator).id != new_node.id) &&
               (open_iterator != st_open.end())) {
            ++open_iterator;
        }
    } else
        open_iterator = st_open.end();
    ...
    if (open_iterator != st_open.end()) {
        ...
        cout << "ouch see this one before: (" << tmp.x_coordinate << " "
             << tmp.y_coordinate << ")" << endl;
        ...
        // cout << "umm this is yammy new one: (" << new_node.x_coordinate << "
        // " << new_node.y_coordinate << ")" << endl;
...
int main(int argc, char **argv) {
    ...
    maze >> n_start.x_coordinate >> n_start.y_coordinate >>
        n_goal.x_coordinate >> n_goal.y_coordinate;
    n_start.id_c();
    n_start.g = 0;
    n_goal.id_c();
    n_goal.h = 0;

    vector<vector<bool>> map(razmer1,
                             vector<bool>(razmer2));  // Initialization of map
    int x, y, b;                                      //
    for (int i = 0; i < razmer1;
         ++i) {  // - coordinates and states of other nodes
    ...
    while (goal_is_not_reached) {
        ...
        cout << "Curently woking on :  " << s.x_coordinate << "  "
             << s.y_coordinate << endl;
